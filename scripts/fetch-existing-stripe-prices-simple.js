// Simple CommonJS version to avoid ES module issues
const fs = require("fs")
const path = require("path")

// Read the products data directly from the file
const productsFilePath = path.join(__dirname, "..", "lib", "products-data.ts")

// Simple function to extract products array from TypeScript file
function extractProductsFromFile() {
  try {
    ...
    // This is a simplified extraction - we'll manually define the products for this script
    const products = [
...
async function fetchExistingStripePrices() {
  ...
  const products = extractProductsFromFile()
